...
export default function UserNavbar() {
  ...
  return (
    <header className="flex items-center justify-between px-6 h-16 bg-white/95 backdrop-blur-xl border-b border-gray-100/50 shadow-lg sticky top-0 z-40">
      {/* Left: Hamburger + Logo */}
      <div className="flex items-center gap-4">
        <motion.button
          onClick={() => setShowMobileMenu(!showMobileMenu)}
          className="p-2.5 text-gray-700 hover:text-gray-900 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 rounded-2xl transition-all duration-300 hover:shadow-lg lg:hidden"
        ...
        >
          <img
            src={UptoSkillsLogo}
            alt="UptoSkills"
            className="w-44 h-11 object-contain shadow-md rounded-xl hover:shadow-lg transition-all duration-300"
          />
        </motion.div>
      </div>
      ...
      <div className="flex items-center gap-6 relative">
        ...
        <div className="relative" ref={notificationDropdownRef}>
          ...
          {/* Toggle handler with console log to verify updated code is loaded */}
...
const NotificationItemDropdown = ({ notif, index, getTypeIcon, getAvatarColor }) => (
  <motion.div
    ...
    className={`px-6 py-5 hover:bg-gradient-to-r hover:from-yellow-50/70 hover:to-amber-50/40 transition-all duration-300 group cursor-pointer border-b border-transparent hover:border-yellow-200/40 ${
      notif.isUnread ? 'bg-gradient-to-r from-yellow-50/40 to-transparent shadow-sm ring-1 ring-yellow-200/30' : ''
    }`}
  ...
  >
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-bold text-sm leading-relaxed mb-2.5 line-clamp-2 transition-all duration-300 group-hover:line-clamp-none ${
          notif.isUnread 
